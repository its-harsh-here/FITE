...
# 8.1 Direct Presigned Chunk Upload & Commit (Direct-to-B2)

In production with cloud object storage (e.g. Backblaze B2), the browser uploads chunks directly to object storage using short-lived presigned URLs generated by the backend control plane.

### Step 1: Request Upload URL
```http
POST /api/transfers/{transferId}/chunks/{chunkIndex}/upload-url
Content-Type: application/json

{
  "checksum": "sha256:...",
  "md5Checksum": "<base64-or-hex-md5>",
  "size": 8388608
}
```
...

{
  ...
  "storageKey": "transfers/{transferId}/chunks/{chunkIndex:06d}_{checksum}",
  "headers": {
    "Content-MD5": "<base64-md5>"
  },
  "expiresAt": "2026-09-05T18:15:00Z"
}
```
...
### Step 3: Commit Chunk to Backend
```http
POST /api/transfers/{transferId}/chunks/{chunkIndex}/commit
Content-Type: application/json
...
# 11.1 Direct Presigned Chunk Download (Direct-to-B2)

In production with cloud object storage, the receiver requests a short-lived presigned GET URL from the control plane and streams chunk bytes directly from Backblaze B2.

### Step 1: Request Download URL
```http
GET /api/transfers/{transferId}/chunks/{chunkIndex}/download-url?token={shareToken}
```
...

{
  ...
  "size": 8388608,
  "checksum": "sha256:...",
  "downloadUrl": "https://s3.us-east-005.backblazeb2.com/...",
  "expiresAt": "2026-09-05T18:15:00Z"
}
```

### Step 2: Receiver Downloads Directly from Storage
```http
GET <downloadUrl>
```

The receiver calculates SHA-256 over the downloaded chunk bytes and verifies that it matches the authoritative `checksum` field before committing the chunk to local storage.
